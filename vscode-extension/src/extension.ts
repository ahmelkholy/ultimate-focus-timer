...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('ultimateFocusTimer.startSession', async () => {
            await startFocusSession();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('ultimateFocusTimer.stopSession', async () => {
            await stopFocusSession();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('ultimateFocusTimer.showStatus', async () => {
    ...
    try {
        ...
        if (currentStatus.phase === 'idle') {
            ...
            const phaseEmoji = {
                ...
                'deep_work': '🎯',
                'neural_rest': '🧘'
            }[currentStatus.phase] || '⏱️';

            vscode.window.showInformationMessage(
                `${phaseEmoji} ${currentStatus.phase.toUpperCase()} - ${timeStr} remaining`
            );
        }
    } catch (error) {
        handleDaemonError(error);
